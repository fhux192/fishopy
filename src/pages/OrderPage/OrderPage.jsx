    ...
    <div className={styles.container}>
      {
        <>
          <Card
            className={`${styles.cardStep} bg-Black mt-[3rem] mx-2 lg:mt-[8rem]`}
          >
            <Steps
              size="small"
              current={currentStep}
          ...
          <div>
            {currentStep == 2 && (
              <Result
                icon={<SmileOutlined style={{ color: "white" }} />}
                title={<span className="text-white">Đặt hành thành công!</span>}
                extra={
                  <Link to={"/"}>
          ...
          {currentStep == 0 && (
            <CheckoutOrder setCurrentStep={setCurrentStep} />
          )}
          {currentStep == 1 && (
            <CheckoutPayment
              addressDelivery={addressDelivery}
