        ...
        <div className="bg-white shadow-lg rounded-lg w-full max-w-6xl p-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="flex flex-col gap-6">
              ...
              <p className="text-2xl text-red-500">Giá: {product.price}</p>
              <div className="flex items-center gap-2">
                <span className="text-lg">Tình trạng:</span>
                <span
                  className={`${
                    product.status === "Còn Hàng" ? "text-green-500" : "text-red-500"
                  } text-lg font-semibold`}
                >
                  {product.status}
                </span>
              </div>
              <div className="bg-gray-100 p-4 rounded shadow-inner overflow-y-auto h-40">
                <h2 className="text-xl font-semibold mb-2">Giới Thiệu</h2>
                <p className="text-md">{product.introduction}</p>
              </div>
              <button className="bg-secondTeal hover:bg-teal-500 text-black font-bold py-3 px-3 rounded-full transition duration-300">
                THÊM VÀO GIỎ HÀNG
              </button>
              <button
                onClick={handlePaymentClick}
                className="bg-primaryBlack hover:bg-teal-700 text-white font-bold py-3 px-3 rounded-full transition duration-300"
              >
                THANH TOÁN
              </button>
            </div>
          </div>
          <div className="mt-10 border-2 border-gray-300 p-6 rounded-lg">
            <h2 className="text-3xl font-bold mb-4 text-center">Mô Tả</h2>
