import fish1 from "../../../assets/fishCardImg/fish (1).png";
import fish2 from "../../../assets/fishCardImg/fish (2).png";
import fish3 from "../../../assets/fishCardImg/fish (3).png";
import fish4 from "../../../assets/fishCardImg/fish (4).png";
import fish5 from "../../../assets/fishCardImg/fish (5).png";
import fish6 from "../../../assets/fishCardImg/fish (6).png";
import fish7 from "../../../assets/fishCardImg/fish (7).png";
import fish8 from "../../../assets/fishCardImg/fish (8).png";
import fish9 from "../../../assets/fishCardImg/fish (9).png";
import fish10 from "../../../assets/fishCardImg/fish (10).png";
import fish11 from "../../../assets/fishCardImg/fish (11).png";
import fish12 from "../../../assets/fishCardImg/fish (12).png";
import fish13 from "../../../assets/fishCardImg/fish (13).png";
import fish14 from "../../../assets/fishCardImg/fish (14).png";
import fish15 from "../../../assets/fishCardImg/fish (15).png";
import fish16 from "../../../assets/fishCardImg/fish (16).png";
import fish17 from "../../../assets/fishCardImg/fish (17).png";
import fish18 from "../../../assets/fishCardImg/fish (18).png";
import fish19 from "../../../assets/fishCardImg/fish (19).png";
import fish20 from "../../../assets/fishCardImg/fish (20).png";
import fish21 from "../../../assets/fishCardImg/fish (21).png";
import fish22 from "../../../assets/fishCardImg/fish (22).png";
import fish23 from "../../../assets/fishCardImg/fish (23).png";
import fish24 from "../../../assets/fishCardImg/fish (24).png";
import fish25 from "../../../assets/fishCardImg/fish (25).png";
import fish26 from "../../../assets/fishCardImg/fish (26).png";
import fish27 from "../../../assets/fishCardImg/fish (27).png";
import fish28 from "../../../assets/fishCardImg/fish (28).png";
import fish29 from "../../../assets/fishCardImg/fish (29).png";
import fish31 from "../../../assets/fishCardImg/fish (31).png";
import fish32 from "../../../assets/fishCardImg/fish (32).png";
import fish33 from "../../../assets/fishCardImg/fish (33).png";
import fish34 from "../../../assets/fishCardImg/fish (34).png";
import fish35 from "../../../assets/fishCardImg/fish (35).png";
import fish36 from "../../../assets/fishCardImg/fish (36).png";
import fish37 from "../../../assets/fishCardImg/fish (37).png";
import fish38 from "../../../assets/fishCardImg/fish (38).png";
import fish39 from "../../../assets/fishCardImg/fish (39).png";
import fish40 from "../../../assets/fishCardImg/fish (40).png";
import fish41 from "../../../assets/fishCardImg/fish (41).png";
import fish42 from "../../../assets/fishCardImg/fish (42).png";
import fish43 from "../../../assets/fishCardImg/fish (43).png";
import fish44 from "../../../assets/fishCardImg/fish (44).png";
import fish45 from "../../../assets/fishCardImg/fish (45).png";
import fish46 from "../../../assets/fishCardImg/fish (46).png";
import fish47 from "../../../assets/fishCardImg/fish (47).png";
import fish48 from "../../../assets/fishCardImg/fish (48).png";

export const ProductsData = [
  {
    id: 1,
    cardImg: fish1,
    proImg: fish1,
    title: "Ca Vang 1",
    price: "100.000đ",
    description: "Cá vằng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá vằng là loài cá biển phổ biến, có thân hình dài và ít xương, thích hợp với nhiều món ăn khác nhau.",
  },
  {
    id: 2,
    cardImg: fish2,
    proImg: fish2,
    title: "Ca Xanh 2",
    price: "100.000đ",
    description: "Cá xanh tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá xanh là loài cá biển có màu da đặc trưng, có thể được nướng, hấp hoặc luộc với nhiều loại gia vị khác nhau.",
  },
  {
    id: 3,
    cardImg: fish3,
    proImg: fish3,
    title: "Ca Cam 3",
    price: "100.000đ",
    description: "Cá các tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá các là loài cá biển có vẻ đẹp, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 4,
    cardImg: fish4,
    proImg: fish4,
    title: "Ca Den 4",
    price: "100.000đ",
    description: "Cá đen tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá đen là loài cá biển có màu da đen đậm, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
  {
    id: 5,
    cardImg: fish5,
    proImg: fish5,
    title: "Ca Bien 5",
    price: "100.000đ",
    description: "Cá biển tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá biển là một loại cá rất phổ biến, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 6,
    cardImg: fish6,
    proImg: fish6,
    title: "Ca Trang 6",
    price: "100.000đ",
    description: "Cá trắng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá trắng là loài cá biển có màu da trắng tuyệt đẹp, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
  {
    id: 7,
    cardImg: fish7,
    proImg: fish7,
    title: "Ca Vang 7",
    price: "100.000đ",
    description: "Cá vằng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá vằng là loài cá biển phổ biến, có thân hình dài và ít xương, thích hợp với nhiều món ăn khác nhau.",
  },
  {
    id: 8,
    cardImg: fish8,
    proImg: fish8,
    title: "Ca Xanh 8",
    price: "100.000đ",
    description: "Cá xanh tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá xanh là loài cá biển có màu da đặc trưng, có thể được nướng, hấp hoặc luộc với nhiều loại gia vị khác nhau.",
  },
  {
    id: 9,
    cardImg: fish9,
    proImg: fish9,
    title: "Ca Cam 9",
    price: "100.000đ",
    description: "Cá các tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá các là loài cá biển có vẻ đẹp, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 10,
    cardImg: fish10,
    proImg: fish10,
    title: "Ca Den 10",
    price: "100.000đ",
    description: "Cá đen tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá đen là loài cá biển có màu da đen đậm, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
  {
    id: 11,
    cardImg: fish11,
    proImg: fish11,
    title: "Ca Bien 11",
    price: "100.000đ",
    description: "Cá biển tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá biển là một loại cá rất phổ biến, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 12,
    cardImg: fish12,
    proImg: fish12,
    title: "Ca Trang 12",
    price: "100.000đ",
    description: "Cá trắng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá trắng là loài cá biển có màu da trắng tuyệt đẹp, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
  {
    id: 13,
    cardImg: fish13,
    proImg: fish13,
    title: "Ca Vang 13",
    price: "100.000đ",
    description: "Cá vằng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá vằng là loài cá biển phổ biến, có thân hình dài và ít xương, thích hợp với nhiều món ăn khác nhau.",
  },
  {
    id: 14,
    cardImg: fish14,
    proImg: fish14,
    title: "Ca Xanh 14",
    price: "100.000đ",
    description: "Cá xanh tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá xanh là loài cá biển có màu da đặc trưng, có thể được nướng, hấp hoặc luộc với nhiều loại gia vị khác nhau.",
  },
  {
    id: 15,
    cardImg: fish15,
    proImg: fish15,
    title: "Ca Cam 15",
    price: "100.000đ",
    description: "Cá các tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá các là loài cá biển có vẻ đẹp, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 16,
    cardImg: fish16,
    proImg: fish16,
    title: "Ca Den 16",
    price: "100.000đ",
    description: "Cá đen tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá đen là loài cá biển có màu da đen đậm, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
  {
    id: 17,
    cardImg: fish17,
    proImg: fish17,
    title: "Ca Bien 17",
    price: "100.000đ",
    description: "Cá biển tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá biển là một loại cá rất phổ biến, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 18,
    cardImg: fish18,
    proImg: fish18,
    title: "Ca Trang 18",
    price: "100.000đ",
    description: "Cá trắng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá trắng là loài cá biển có màu da trắng tuyệt đẹp, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
  {
    id: 19,
    cardImg: fish19,
    proImg: fish19,
    title: "Ca Vang 19",
    price: "100.000đ",
    description: "Cá vằng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá vằng là loài cá biển phổ biến, có thân hình dài và ít xương, thích hợp với nhiều món ăn khác nhau.",
  },
  {
    id: 20,
    cardImg: fish20,
    proImg: fish20,
    title: "Ca Xanh 20",
    price: "100.000đ",
    description: "Cá xanh tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá xanh là loài cá biển có màu da đặc trưng, có thể được nướng, hấp hoặc luộc với nhiều loại gia vị khác nhau.",
  },
  {
    id: 21,
    cardImg: fish21,
    proImg: fish21,
    title: "Ca Cam 21",
    price: "100.000đ",
    description: "Cá các tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá các là loài cá biển có vẻ đẹp, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 22,
    cardImg: fish22,
    proImg: fish22,
    title: "Ca Den 22",
    price: "100.000đ",
    description: "Cá đen tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá đen là loài cá biển có màu da đen đậm, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
  {
    id: 23,
    cardImg: fish23,
    proImg: fish23,
    title: "Ca Bien 23",
    price: "100.000đ",
    description: "Cá biển tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá biển là một loại cá rất phổ biến, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 24,
    cardImg: fish24,
    proImg: fish24,
    title: "Ca Trang 24",
    price: "100.000đ",
    description: "Cá trắng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá trắng là loài cá biển có màu da trắng tuyệt đẹp, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
  {
    id: 25,
    cardImg: fish25,
    proImg: fish25,
    title: "Ca Vang 25",
    price: "100.000đ",
    description: "Cá vằng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá vằng là loài cá biển phổ biến, có thân hình dài và ít xương, thích hợp với nhiều món ăn khác nhau.",
  },
  {
    id: 26,
    cardImg: fish26,
    proImg: fish26,
    title: "Ca Xanh 26",
    price: "100.000đ",
    description: "Cá xanh tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá xanh là loài cá biển có màu da đặc trưng, có thể được nướng, hấp hoặc luộc với nhiều loại gia vị khác nhau.",
  },
  {
    id: 27,
    cardImg: fish27,
    proImg: fish27,
    title: "Ca Cam 27",
    price: "100.000đ",
    description: "Cá các tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá các là loài cá biển có vẻ đẹp, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 28,
    cardImg: fish28,
    proImg: fish28,
    title: "Ca Den 28",
    price: "100.000đ",
    description: "Cá đen tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá đen là loài cá biển có màu da đen đậm, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
  {
    id: 29,
    cardImg: fish29,
    proImg: fish29,
    title: "Ca Bien 29",
    price: "100.000đ",
    description: "Cá biển tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá biển là một loại cá rất phổ biến, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 31,
    cardImg: fish31,
    proImg: fish31,
    title: "Ca Vang 31",
    price: "100.000đ",
    description: "Cá vằng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá vằng là loài cá biển phổ biến, có thân hình dài và ít xương, thích hợp với nhiều món ăn khác nhau.",
  },
  {
    id: 32,
    cardImg: fish32,
    proImg: fish32,
    title: "Ca Xanh 32",
    price: "100.000đ",
    description: "Cá xanh tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá xanh là loài cá biển có màu da đặc trưng, có thể được nướng, hấp hoặc luộc với nhiều loại gia vị khác nhau.",
  },
  {
    id: 33,
    cardImg: fish33,
    proImg: fish33,
    title: "Ca Cam 33",
    price: "100.000đ",
    description: "Cá các tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá các là loài cá biển có vẻ đẹp, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 34,
    cardImg: fish34,
    proImg: fish34,
    title: "Ca Den 34",
    price: "100.000đ",
    description: "Cá đen tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá đen là loài cá biển có màu da đen đậm, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
  {
    id: 35,
    cardImg: fish35,
    proImg: fish35,
    title: "Ca Bien 35",
    price: "100.000đ",
    description: "Cá biển tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá biển là một loại cá rất phổ biến, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 36,
    cardImg: fish36,
    proImg: fish36,
    title: "Ca Trang 36",
    price: "100.000đ",
    description: "Cá trắng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá trắng là loài cá biển có màu da trắng tuyệt đẹp, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
  {
    id: 37,
    cardImg: fish37,
    proImg: fish37,
    title: "Ca Vang 37",
    price: "100.000đ",
    description: "Cá vằng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá vằng là loài cá biển phổ biến, có thân hình dài và ít xương, thích hợp với nhiều món ăn khác nhau.",
  },
  {
    id: 38,
    cardImg: fish38,
    proImg: fish38,
    title: "Ca Xanh 38",
    price: "100.000đ",
    description: "Cá xanh tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá xanh là loài cá biển có màu da đặc trưng, có thể được nướng, hấp hoặc luộc với nhiều loại gia vị khác nhau.",
  },
  {
    id: 39,
    cardImg: fish39,
    proImg: fish39,
    title: "Ca Cam 39",
    price: "100.000đ",
    description: "Cá các tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá các là loài cá biển có vẻ đẹp, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 40,
    cardImg: fish40,
    proImg: fish40,
    title: "Ca Den 40",
    price: "100.000đ",
    description: "Cá đen tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá đen là loài cá biển có màu da đen đậm, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
  {
    id: 41,
    cardImg: fish41,
    proImg: fish41,
    title: "Ca Bien 41",
    price: "100.000đ",
    description: "Cá biển tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá biển là một loại cá rất phổ biến, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 42,
    cardImg: fish42,
    proImg: fish42,
    title: "Ca Trang 42",
    price: "100.000đ",
    description: "Cá trắng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá trắng là loài cá biển có màu da trắng tuyệt đẹp, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
  {
    id: 43,
    cardImg: fish43,
    proImg: fish43,
    title: "Ca Vang 43",
    price: "100.000đ",
    description: "Cá vằng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá vằng là loài cá biển phổ biến, có thân hình dài và ít xương, thích hợp với nhiều món ăn khác nhau.",
  },
  {
    id: 44,
    cardImg: fish44,
    proImg: fish44,
    title: "Ca Xanh 44",
    price: "100.000đ",
    description: "Cá xanh tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá xanh là loài cá biển có màu da đặc trưng, có thể được nướng, hấp hoặc luộc với nhiều loại gia vị khác nhau.",
  },
  {
    id: 45,
    cardImg: fish45,
    proImg: fish45,
    title: "Ca Cam 45",
    price: "100.000đ",
    description: "Cá các tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá các là loài cá biển có vẻ đẹp, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 46,
    cardImg: fish46,
    proImg: fish46,
    title: "Ca Den 46",
    price: "100.000đ",
    description: "Cá đen tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá đen là loài cá biển có màu da đen đậm, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
  {
    id: 47,
    cardImg: fish47,
    proImg: fish47,
    title: "Ca Bien 47",
    price: "100.000đ",
    description: "Cá biển tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá biển là một loại cá rất phổ biến, có thể được chế biến thành nhiều món ăn ngon tuyệt vời.",
  },
  {
    id: 48,
    cardImg: fish48,
    proImg: fish48,
    title: "Ca Trang 48",
    price: "100.000đ",
    description: "Cá trắng tươi, ngon, giàu dinh dưỡng.",
    status: "new",
    introduction:
      "Cá trắng là loài cá biển có màu da trắng tuyệt đẹp, có thể được chế biến thành nhiều món ăn hấp dẫn.",
  },
];
