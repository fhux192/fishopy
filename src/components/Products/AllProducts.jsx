...
const AllProducts = () => {
  ...
  return (
    <div>
      {/*Header Section*/}
      <div className=" text-lg mb-[5.5rem] h-[1.8rem] max-w-[1000rem]">
      ...
      <div className="mx-0 lg:mx-[4.5rem]">
        <div className=" grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 place-items-center mx-[1.5rem] lg:mx-0 mx-20">
          ...
          {currentPageProducts.map((data) => (
            <div
              key={data.id}
              className="group mb-[5rem] h-[14rem] border-b-primaryBlack shadow-lg shadow-primaryGrey hover:shadow-teal-700 rounded-3xl cursor-pointer"
            >
              <LazyLoadImage
                src={data.cardImg}
                alt=""    
